...
LearningEngineFeaturewise::LearningEngineFeaturewise(gem5::prefetch::Base *parent, float alpha, float gamma, float epsilon, uint32_t actions, uint64_t seed, std::string policy, std::string type, bool zero_init)
...
void LearningEngineFeaturewise::plot_scores()
{
	...
	for(uint32_t index = 0; index < knob::le_featurewise_plot_actions.size(); ++index)
	{
		...
		int action = 0;//scooby->getAction(knob::le_featurewise_plot_actions[index]);
		stringstream ss;
		if(action > 0)
			ss << "+" << action;
		else
