...
LearningEngineBase::LearningEngineBase(gem5::prefetch::Base *parent, float alpha, float gamma, float epsilon, uint32_t actions, uint32_t states, uint64_t seed, std::string policy, std::string type)
	: m_parent(parent)
	, m_alpha(alpha)
	, m_gamma(gamma)
